...
public class HibernateUtils {
	...
	public static <T extends UUIDEntity> void mergeByCheckedIds2(final Collection<T> srcObjects,
	...
	public static String propertyFilter2String(List<PropertyFilter> filters){
		if(filters!=null){
			return buildHqlWhereParamStr(filters);
		}
		return null;
	}
	public  static String  buildHqlWhereParamStr(List<PropertyFilter> filters){
		String  str="";
		for (PropertyFilter filter : filters) {
			if (!filter.hasMultiProperties()) { 
				String s = buildHqlParam(filter.getPropertyName(), filter.getMatchValue(), filter.getMatchType());
				if(!StringUtil.isEmpty(s))
				 str += " " +s+" and";
			} else {
				for (String param : filter.getPropertyNames()) {
					  str += " "+buildHqlParam(param,	filter.getMatchValue(), filter.getMatchType()) +" and";
				}
			}
		}
		if(str.equals(""))str="1=1";
		else{
			str = str.substring(0,str.length()-3);
		}
		return str;
	}
	public static String  buildHqlParam(final String propertyName,
			final Object propertyValue, final MatchType matchType) {
			Object param = null;
		  if (propertyValue instanceof Integer) {
			  	param = Integer.parseInt(propertyValue.toString());
				   ...
				   param = StringUtil.dateToStringWithHour((Date)propertyValue);
			   } 
		String str="";
		...
		switch (matchType) {
		case EQ:
			str = propertyName +" = "+"'"+param+"'";
			break;
		case LIKE:
			str = propertyName +" like "+"'%"+param+"%'";
			break;
		case LE:
			str = propertyName +" <= "+"'"+param+"'";
			break;
