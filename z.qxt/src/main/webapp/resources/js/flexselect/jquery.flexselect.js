 ...
 * 
 * For usage and examples, visit: http://rmm5t.github.com/jquery-flexselect/
 * 
...
(function($) {
	...
	$.extend($.flexselect.prototype, {
		settings : {
			allowMismatch : false,
			allowMismatchBlank : true, // If "true" a user can backspace such
										// that the value is nothing (even if no
										// blank value was provided in the
										// original criteria)
		...
		renderControls : function() {
			var selected = this.settings.preSelection ? this.select
					.children("option:selected") : null;
			...
			this.input = $("<input type='text' autocomplete='off' />").attr(
					{
						id : this.settings.inputIdTransform(this.select
								.attr("id")),
						name : this.settings.inputNameTransform(this.select
								.attr("name")),
						accesskey : this.select.attr("accesskey"),
						tabindex : this.select.attr("tabindex"),
						style : this.select.attr("style")
					}).addClass(this.select.attr("class")).val(
					$.trim(selected ? selected.text() : '')).css({
				visibility : 'visible'
			});
			if($('#'+this.settings.dropdownIdTransform(this.select.attr("id")))){
				$('#'+this.settings.dropdownIdTransform(this.select.attr("id"))).remove();
			}
		...
		wire : function() {
			...
			this.input.blur(function() {
				if (!self.dropdownMouseover) {
					self.hide();
					if (self.settings.allowMismatchBlank
							&& $.trim($(this).val()) == '')
		...
		filterResults : function() {
			...
			if (this.settings.mustMatch == false && results.length == 0) {
				results.push({
					name : abbreviation,
					value : abbreviation,
					score : 0.0
		...
		renderDropdown : function() {
			var dropdownBorderWidth = this.dropdown.outerWidth()
					- this.dropdown.innerWidth();
			...
			$.each(this.results, function() {
				// html += '<li>' + this.name + ' <small>[' +
				// Math.round(this.score*100)/100 + ']</small></li>';
		...
		adjustMaxHeight : function() {
			...
			this.dropdown.css('max-height', top > maxTop ? (Math.max(0, maxTop
					- top + this.dropdown.innerHeight()) + 'px') : '');
		},
		...
		markSelected : function(n) {
			...
			if (delta > 0) {
				...
				this.allowMouseMove = false;
				this.dropdown.scrollTop(Math.max(0, this.dropdown.scrollTop()
						+ top));
			}
		},
		...
		pickSelected : function() {
			...
			if (selected) {
				...
				this.picked = true;
				this.settings.callback(selected.value);
