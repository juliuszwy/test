 ...
 * LiquidMetal.
 * 
 * For usage and examples, visit: http://github.com/rmm5t/liquidmetal
 * 
...
var LiquidMetal = (function() {
	...
	return {
		...
		score : function(string, abbrev) {
			...
			if (abbrev.length > string.length)
		...
		_scoreAll : function(string, search, abbrev, searchIndex, abbrIndex,
			...
			if (abbrIndex == abbrev.length) {
				...
				var trailScore = started ? SCORE_TRAILING_BUT_STARTED
						: SCORE_TRAILING;
			...
			while ((index = search.indexOf(c, searchIndex + 1)) != -1) {
				...
				searchIndex = index;
				this._scoreAll(string, search, abbrev, searchIndex, abbrIndex,
						scores, allScores);
			}
		}
	};
